...
The QC status of the library and library aliquot indicate library preparation QC, whereas run-item QC statuses can apply to run-libraries 
(a specific library in a specific lane of a specific sequencing run) or array run–samples (a specific sample in a specific array run). 
This means that a library prep may pass QC, a run may pass QC, the partition may even pass QC, but the data for a particular library in that partition may
still be bad.

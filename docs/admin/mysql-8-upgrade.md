...
version of MISO, you must switch to MySQL 8.0. Separate instructions are provided below for bare
metal and Docker Compose setups.
...
1. remove MySQL 5.7, install MySQL 8
...
1. Update the database URL in your `ROOT.xml`. Replace `localhost` and `lims` with the correct host
  and database name. Note the recommended connection options have been changed
...
1. Follow the usual [update procedure](https://miso-lims.readthedocs.io/projects/docs/en/latest/admin/baremetal-installation-guide/#installing-and-upgrading),
  but before running Flyway migrate as usual, run Flyway repair
...
* you should add your usual parameters to all `docker-compose` commands below (--env-file, -f, etc.)
...
1. Update your docker-compose file to use new MISO version and MySQL 8.0
    ...
    * **flyway** service:
      * update image to `ghcr.io/miso-lims/miso-lims-migration:1.57.0`
      * remove secret: `lims_password`
      * add secret: `root_password`
    * **webapp** service:
      * update image to `ghcr.io/miso-lims/miso-lims-webapp:1.57.0`
1. Spin up MySQL container only, with new mounted volume

    ```
    docker-compose up db
    ```

1. Restore tables-only backup to the new container using root user

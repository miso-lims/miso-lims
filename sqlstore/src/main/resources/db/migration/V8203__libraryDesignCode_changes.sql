...
ALTER TABLE LibraryAdditionalInfo ADD COLUMN `libraryDesignCode` BIGINT(20);
ALTER TABLE LibraryAdditionalInfo ADD CONSTRAINT `FK_lai_libraryDesignCode_libraryDesignCodeId` FOREIGN KEY (`libraryDesignCode`) REFERENCES `LibraryDesignCode` (`libraryDesignCodeId`);
ALTER TABLE LibraryDesign ADD COLUMN `libraryDesignCode` BIGINT(20);
ALTER TABLE LibraryDesign ADD CONSTRAINT `FK_ld_libraryDesignCode_libraryDesignCodeId` FOREIGN KEY (`libraryDesignCode`) REFERENCES `LibraryDesignCode` (`libraryDesignCodeId`);
ALTER TABLE LibraryDesign DROP KEY `uk_libraryDesign_name`;
  ...
  OPEN updateLibraryDesignCodeCursor;
    update_LDC: LOOP
      ...
      IF v_finished = 1 THEN LEAVE update_LDC; END IF;
	    ...
	    UPDATE LibraryDesign SET libraryDesignCode = v_ldcId WHERE SUBSTRING(name, 1, 2) = @code;
  	    -- infer libraryDesignCode from alias
	    UPDATE LibraryAdditionalInfo SET libraryDesignCode = v_ldcId WHERE libraryId IN (SELECT libraryId FROM Library WHERE RIGHT(alias, 2) = @code);
	    ...
	    UPDATE LibraryAdditionalInfo SET libraryDesignCode = v_ldcId WHERE libraryDesign IN (SELECT libraryDesignId FROM LibraryDesign WHERE SUBSTRING(name, 1, 2) = @code);
    END LOOP update_LDC;
...
ALTER TABLE LibraryAdditionalInfo MODIFY COLUMN `libraryDesignCode` BIGINT(20) NOT NULL;
ALTER TABLE LibraryDesign MODIFY COLUMN `libraryDesignCode` BIGINT(20) NOT NULL;

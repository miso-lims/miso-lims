...
FOR EACH ROW
  BEGIN
    ...
    SET log_message = CONCAT_WS(', ',
      ...
      CASE WHEN NEW.concentrationUnits <> OLD.concentrationUnits THEN CONCAT(NEW.name, ' concentration units: ', OLD.concentrationUnits, ' → ', NEW.concentrationUnits) END);
    IF log_message IS NOT NULL AND log_message <> '' THEN
      ...
      NEW.library_libraryId,
        COALESCE(CONCAT_WS(',',
          ...
          CASE WHEN NEW.concentrationUnits <> OLD.concentrationUnits THEN CONCAT(NEW.name, ' concentrationUnits') END
        ), ''),
        NEW.lastModifier,
        log_message,
        NEW.lastUpdated

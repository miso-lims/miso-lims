...
public class SQLLibraryDAO implements LibraryStore {
  ...
  public static final String LIBRARY_UPDATE = "UPDATE "
      + TABLE_NAME
  ...
  @TriggersRemove(cacheName = { "libraryCache", "lazyLibraryCache" }, keyGenerator = @KeyGenerator(name = "HashCodeCacheKeyGenerator", properties = {
    ...
    if (library.getId() == AbstractLibrary.UNSAVED_ID) {
      ...
      try {
        if (libraryNamingScheme.validateField("name", library.getName()) && libraryNamingScheme.validateField("alias", library.getAlias())) {
          ...
          params.addValue("identificationBarcode", library.getName() + "::" + library.getAlias());
  ...
  public List<Library> listBySearch(String query) {
    ...
    return template.query(LIBRARIES_SELECT_BY_SEARCH, new Object[] { mySQLQuery, mySQLQuery, mySQLQuery, mySQLQuery }, new LibraryMapper(
        true));
  }

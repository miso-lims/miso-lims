...
public class SQLSequencerPartitionContainerDAO implements SequencerPartitionContainerStore {
  ...
  public synchronized long save(SequencerPartitionContainer<SequencerPoolPartition> sequencerPartitionContainer) throws IOException {
    ...
    if (sequencerPartitionContainer.getId() == AbstractSequencerPartitionContainer.UNSAVED_ID) {
       ...
       * 
       * Number newId = insert.executeAndReturnKey(params); if (newId.longValue() != sequencerPartitionContainer.getId()) { log.error(
       * "Expected SequencerPartitionContainer ID doesn't match returned value from database insert: rolling back..."); new
       * NamedParameterJdbcTemplate(template).update(SEQUENCER_PARTITION_CONTAINER_DELETE, new
       * MapSqlParameterSource().addValue("containerId", newId.longValue())); throw new IOException(
       * "Something bad happened. Expected SequencerPartitionContainer ID doesn't match returned value from DB insert"); } } else { throw
       * new IOException("Cannot save SequencerPartitionContainer - invalid field:" + sequencerPartitionContainer.toString()); }
       * 
   ...
   * lookupCache(cacheManager).get(DbUtils.hashCodeCacheKeyFor(id))) != null) { log.debug(
   * "Cache hit on map for SequencerPartitionContainer " + id); return (T)element.getObjectValue(); } }
   * 

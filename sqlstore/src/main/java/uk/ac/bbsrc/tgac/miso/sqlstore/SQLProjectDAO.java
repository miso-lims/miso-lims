...
public class SQLProjectDAO implements ProjectStore {
	...
	public static final String PROJECTS_SELECT_BY_SEARCH = PROJECTS_SELECT + " WHERE " + "name LIKE ? OR "
			+ "alias LIKE ? OR " + "description LIKE ? ";
	...
	// OVERVIEWS
	public static final String OVERVIEWS_SELECT = "SELECT p.project_projectId, " + "po.overviewId, "
			+ "po.principalInvestigator, " + "po.startDate, " + "po.endDate, " + "po.numProposedSamples, "
			+ "po.locked, " + "po.lastUpdated, " + "po.allSampleQcPassed, " + "po.libraryPreparationComplete, "
			+ "po.allLibraryQcPassed, " + "po.allPoolsConstructed, " + "po.allRunsCompleted, "
	...
	public static final String OVERVIEW_UPDATE = "UPDATE ProjectOverview "
			+ "SET principalInvestigator=:principalInvestigator, " + "startDate=:startDate, " + "endDate=:endDate, "
			+ "numProposedSamples=:numProposedSamples, " + "locked=:locked, " + "allSampleQcPassed=:allSampleQcPassed, "
			+ "libraryPreparationComplete=:libraryPreparationComplete, " + "allLibraryQcPassed=:allLibraryQcPassed, "
			+ "allPoolsConstructed=:allPoolsConstructed, " + "allRunsCompleted=:allRunsCompleted, "
			...
			try {
				...
				if (namingScheme.validateField("name", project.getName())) {
					...
					if (newId.longValue() != project.getId()) {
						log.error("Expected Project ID ('" + project.getId() + "') doesn't match returned value ('"
								+ newId.longValue() + "') from database insert: rolling back...");
						...
						throw new IOException(
								"Something bad happened. Expected Project ID doesn't match returned value from DB insert");
					}
				} else {
	...
	public long saveOverview(ProjectOverview overview) throws IOException {
		...
		if (overview.getId() == ProjectOverview.UNSAVED_ID) {
			SimpleJdbcInsert insert = new SimpleJdbcInsert(template).withTableName("ProjectOverview")
	...
	public boolean remove(Project project) throws IOException {
		NamedParameterJdbcTemplate namedTemplate = new NamedParameterJdbcTemplate(template);
		boolean ok = true;
		if (project.isDeletable() && (namedTemplate.update(PROJECT_DELETE,
	...
	public boolean removeOverview(ProjectOverview overview) throws IOException {
		NamedParameterJdbcTemplate namedTemplate = new NamedParameterJdbcTemplate(template);
		return (overview.isDeletable() && (namedTemplate.update(OVERVIEW_DELETE,
	...
	public Project lazyGet(long projectId) throws IOException {
		List<Project> eResults = template.query(PROJECT_SELECT_BY_ID, new Object[] { projectId },
	...
	public List<Project> listBySearch(String query) {
		...
		return template.query(PROJECTS_SELECT_BY_SEARCH, new Object[] { mySQLQuery, mySQLQuery, mySQLQuery },
				new ProjectMapper(true));
	}
	...
	public Project getByStudyId(long studyId) throws IOException {
		List<Project> eResults = template.query(PROJECT_SELECT_BY_STUDY_ID, new Object[] { studyId },
				new ProjectMapper());
		return eResults.size() > 0 ? eResults.get(0) : null;
	}
	...
	public ProjectOverview getProjectOverviewById(long overviewId) throws IOException {
		List<ProjectOverview> eResults = template.query(OVERVIEW_SELECT_BY_ID, new Object[] { overviewId },
				new ProjectOverviewMapper());
		return eResults.size() > 0 ? eResults.get(0) : null;
	}

	public ProjectOverview lazyGetProjectOverviewById(long overviewId) throws IOException {
		List<ProjectOverview> eResults = template.query(OVERVIEW_SELECT_BY_ID, new Object[] { overviewId },
				new ProjectOverviewMapper(true));
		return eResults.size() > 0 ? eResults.get(0) : null;
	}

	@Override
	public List<ProjectOverview> listOverviewsByProjectId(long projectId) throws IOException {
		return template.query(OVERVIEW_SELECT_BY_RELATED_PROJECT, new Object[] { projectId },
				new ProjectOverviewMapper(true));
	}
		...
		public Project mapRow(ResultSet rs, int rowNum) throws SQLException {
			...
			try {
				...
				try {
					...
					project.setWatchers(
							new HashSet<User>(watcherDAO.getWatchersByEntityName(project.getWatchableIdentifier())));
	...
	public class ProjectOverviewMapper extends CacheAwareRowMapper<ProjectOverview> {
		...
		public ProjectOverview mapRow(ResultSet rs, int rowNum) throws SQLException {
			...
			try {
				...
				EntityGroup<ProjectOverview, Sample> osg = entityGroupDAO.getEntityGroupByParent(overview,
						overview.getClass());
				if (osg != null) {
				...
				overview.setWatchers(
						new HashSet<>(watcherDAO.getWatchersByEntityName(overview.getWatchableIdentifier())));

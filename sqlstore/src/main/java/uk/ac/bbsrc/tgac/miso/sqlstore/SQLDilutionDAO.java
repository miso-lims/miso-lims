...
public class SQLDilutionDAO implements DilutionStore {
  ...
  public Collection<? extends Dilution> listAllDilutionsByProjectAndPlatform(long projectId, PlatformType platformType) throws IOException {
    if (platformType.equals(PlatformType.ILLUMINA)) {
      ...
      dils.addAll(template.query(LIBRARY_DILUTION_SELECT_BY_PROJECT_AND_LIBRARY_PLATFORM,
          new Object[] { platformType.getKey(), projectId }, new LazyLibraryDilutionMapper()));
  ...
  public long saveLibraryDilution(LibraryDilution dilution) throws IOException {
    ...
    MapSqlParameterSource params = new MapSqlParameterSource();
    params.addValue("concentration", dilution.getConcentration()).addValue("library_libraryId", dilution.getLibrary().getId())
        .addValue("creationDate", dilution.getCreationDate()).addValue("securityProfile_profileId", securityProfileId)
  ...
  public emPCRDilution getEmPCRDilutionByBarcode(String barcode) throws IOException {
    List eResults = template
        .query(EMPCR_DILUTION_SELECT_BY_IDENTIFICATION_BARCODE, new Object[] { barcode }, new LazyEmPCRDilutionMapper());
    emPCRDilution e = eResults.size() > 0 ? (emPCRDilution) eResults.get(0) : null;
    return e;
  }

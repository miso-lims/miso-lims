...
public class SQLDilutionDAO implements DilutionStore {
  ...
  public static final String EMPCR_DILUTIONS_BY_RELATED_LS454_POOL_ID =
          EMPCR_DILUTION_SELECT_BY_LS454 + " AND p.pool_poolId=?";

  public static final String EMPCR_DILUTIONS_BY_RELATED_SOLID_POOL_ID =
          EMPCR_DILUTION_SELECT_BY_SOLID + " AND p.pool_poolId=?";
  ...
  public Collection<LibraryDilution> listAllLibraryDilutionsByPlatform(PlatformType platformType) throws IOException {
    return template.query(LIBRARY_DILUTION_SELECT_BY_LIBRARY_PLATFORM, new Object[]{platformType.getKey()}, new LazyLibraryDilutionMapper());
  }
  ...
  public Collection<? extends Dilution> listAllDilutionsByProjectAndPlatform(long projectId, PlatformType platformType) throws IOException {
    if (platformType.equals(PlatformType.ILLUMINA)) {
      return template.query(LIBRARY_DILUTION_SELECT_BY_PROJECT_AND_LIBRARY_PLATFORM, new Object[]{platformType.getKey(), projectId}, new LazyLibraryDilutionMapper());
    }
      ...
      dils.addAll(template.query(LIBRARY_DILUTION_SELECT_BY_PROJECT_AND_LIBRARY_PLATFORM, new Object[]{platformType.getKey(), projectId}, new LazyLibraryDilutionMapper()));
      dils.addAll(template.query(EMPCR_DILUTION_SELECT_BY_PROJECT_AND_LIBRARY_PLATFORM, new Object[]{platformType.getKey(), projectId}, new LazyEmPCRDilutionMapper()));
      return dils;
    }
    else {
      return Collections.emptyList();

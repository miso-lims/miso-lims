...
public class DefaultWorkstationService extends AbstractSaveService<Workstation> implements WorkstationService {
  ...
  public ValidationResult validateDeletion(Workstation object) throws IOException {
    ...
    long sopFieldUsage = workstationDao.getUsageByRunSopFieldValues(object);
    if (sopFieldUsage > 0L) {
      result.addError(ValidationError.forDeletionUsage(object, sopFieldUsage, Pluralizer.runs(sopFieldUsage)));
    }
    long sampleSopFieldUsage = workstationDao.getUsageBySampleSopFieldValues(object);
    ...
    long librarySopFieldUsage = workstationDao.getUsageByLibrarySopFieldValues(object);

      ...
      for (Map.Entry<String, Integer> entry : typeMap.entrySet()) {
        ...
        for (Map.Entry<String, Integer> platformEntry : platformMap.entrySet()) {
          ...
          if (libqcpassed > 0 || libqcfailed > 0) {
            overviewRelationArray.add("['" + libraryType + "','" + platform + "'," + libqcpassed + "," + libqcfailed + ","
                + (libqcpassed + libqcfailed) + "]");
    ...
    try {
      ...
      for (Run run : runs) {
        if ((platform.equals("all") || platform.equals(run.getPlatformType().getKey()))
            && (status.equals("all") || (run.getStatus() != null && run.getStatus().getHealth() != null ? status.equals(run.getStatus()
                .getHealth().getKey()) : true))) {
        ...
        if (j.getString("name").equals("runIds")) {
          ...
          if (run != null) {
            ...
            if (run.getStatus() != null && run.getStatus().getHealth() != null) {
              int countQC = statusMap.containsKey(run.getStatus().getHealth().getKey()) ? statusMap.get(run.getStatus().getHealth()
                  .getKey()) : 0;
              countQC++;
              ...
              for (SequencerPoolPartition spp : spc.getPartitions()) {
                if (spp.getPool() != null) {
                  ...
                  if (spp.getPool().getDilutions().size() > 0) {
                    ...
                    for (Dilution dilution : spp.getPool().getDilutions()) {
                      int count = projectMap.containsKey(dilution.getLibrary().getSample().getProject().getName()) ? projectMap
                          .get(dilution.getLibrary().getSample().getProject().getName()) : 0;
                      count++;
      ...
      for (Sample sample : samples) {
        ...
        if (libraries.size() == 0) {
          if (sample.getQcPassed()) {
            samplesArray.add(JSONObject.fromObject("{'name': '" + sample.getName() + "','description':'" + sample.getAlias()
                + "','color': '1'}"));
          } else {
            samplesArray.add(JSONObject.fromObject("{'name': '" + sample.getName() + "','description':'" + sample.getAlias()
                + "','color': '0'}"));

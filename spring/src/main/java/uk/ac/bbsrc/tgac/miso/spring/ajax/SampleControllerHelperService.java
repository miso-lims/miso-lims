...
public class SampleControllerHelperService {
  ...
  public JSONObject bulkAddSampleQCs(HttpSession session, JSONObject json) {
    try {
      ...
      if (ok) {
        ...
        return JSONUtils
            .SimpleJSONError("Failed to add Sample QC to this Library: one of the required fields of the selected QCs is missing or invalid");
      }
    } catch (Exception e) {
      log.error("Failed to add Sample QC to this sample: ", e);
      return JSONUtils.SimpleJSONError("Failed to add Sample QC to this sample: " + e.getMessage());
  ...
  public JSONObject lookupNCBIScientificName(HttpSession session, JSONObject json) {
    ...
    if (taxon != null) {
      ...
      return JSONUtils
          .SimpleJSONError("This scientific name is not of a known taxonomy. You may have problems when trying to submit this data to public repositories.");
    }
  }

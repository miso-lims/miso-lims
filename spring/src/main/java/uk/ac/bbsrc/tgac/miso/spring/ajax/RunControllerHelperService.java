  ...
  public JSONObject changeIlluminaContainer(HttpSession session, JSONObject json) {
    ...
    if ("Illumina MiSeq".equals(instrumentModel) || "Illumina NextSeq 500".equals(instrumentModel)) {
      ...
      for (int i = 0; i < numContainers; i++) {
        ...
        b.append("<table class='in'>");
        b.append("<tr><td>ID:</td><td><button onclick='Run.container.lookupContainer(this, "
            + i
    ...
    try {
      ...
      for (Run run : requestManager.listAllRuns()) {
        jsonArray.add("['"
            ...
            + "','"
            + (run.getStatus() != null && run.getStatus().getStartDate() != null ? LimsUtils
                .getDateAsString(run.getStatus().getStartDate()) : "")
            + "','"
            + (run.getStatus() != null && run.getStatus().getCompletionDate() != null ? LimsUtils.getDateAsString(run.getStatus()
                .getCompletionDate()) : "") + "','" + (run.getPlatformType() != null ? run.getPlatformType().getKey() : "") + "','"

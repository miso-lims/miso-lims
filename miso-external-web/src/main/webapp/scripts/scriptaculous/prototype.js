  ...
  evaluate: function(object) {
    ...
    return this.template.gsub(this.pattern, function(match) {
      ...
      while (match != null) {
        ...
        expr =
        expr.substring('[' == match[3] ? match[1].length : match[0].length);
        match = pattern.exec(expr);
      }
...
Ajax.PeriodicalUpdater = Class.create(Ajax.Base, {
  ...
  updateComplete: function(response) {
    ...
    this.timer =
    this.onTimerEvent.bind(this).delay(this.decay * this.frequency);
  },
...
Element.Methods = {
  ...
  clonePosition: function(element, source) {
    ...
    // set position
    if (options.setLeft)   element.style.left =
                           (p[0] - delta[0] + options.offsetLeft) + 'px';
    if (options.setTop)    element.style.top =
                           (p[1] - delta[1] + options.offsetTop) + 'px';
...
if (Prototype.Browser.Opera) {
  ...
  Element.Methods.getStyle = function(element, style) {
    ...
    if (style == 'opacity') {
      if (value =
          (element.getStyle('filter') || '').match(/alpha\(opacity=(.*)\)/))
    ...
    index: function(parentNode, reverse, ofType) {
      ...
      if (reverse) {
        for (var nodes = parentNode.childNodes, i = nodes.length - 1, j = 1;
          ...
          var node = nodes[i];
          if (node.nodeType == 1 &&
              (!ofType || node._countedByPrototype)) node.nodeIndex = j++;
        }
      }
      else {

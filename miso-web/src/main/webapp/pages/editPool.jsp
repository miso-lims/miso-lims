...
<script type="text/javascript">
  ...
  <c:choose>
  <c:when test="${pool.id != 0}">
    Utils.timer.typewatchFunc(jQuery('#searchElements'), function(){Pool.search.poolSearchElements(jQuery('#searchElements'), '${pool.platformType.key}')}, 300, 2);
  </c:when>
  <c:otherwise>
    Utils.timer.typewatchFunc(jQuery('#searchElements'), function(){Pool.search.poolSearchElements(jQuery('#searchElements'), jQuery('#platformType').val())}, 300, 2);
  </c:otherwise>
  </c:choose>
</script>

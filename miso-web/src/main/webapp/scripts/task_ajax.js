...
function populateRunningTasks() {
  ...
  {'url':ajaxurl},
  {'doOnSuccess':processRunningTasks}
  );
}

var processRunningTasks = function(json) {  jQuery('#runningThrobber').remove();
...
function populatePendingTasks() {
  ...
  {'url':ajaxurl},
  {'doOnSuccess':processPendingTasks}
  );
}

var processPendingTasks = function(json) {
...
function populateCompletedTasks() {
  ...
  {'url':ajaxurl},
  {'doOnSuccess':processCompletedTasks}
  );
}

var processCompletedTasks = function(json) {
...
function populatePipelines() {
  Fluxion.doAjax(
          ...
          'populatePipelines',
  {'url':ajaxurl},
  {'doOnSuccess':processPipelines}
  );
}

var processPipelines = function(json) {
...
function selectPipeline(select, runId) {
  var s = jQuery(select);
  if (!isNullCheck(s.val())) {
    Fluxion.doAjax(
            'taskControllerHelperService',
            'getPipeline',
    {'url':ajaxurl, 'pipeline':s.val(), 'runId':runId},
    {'doOnSuccess':processPipeline}
    );
  }
}

var processPipeline = function(json) {
  if (json.pipeline) {

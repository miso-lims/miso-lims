...
BulkTarget.qc = (function() {
  ...
  return {
    ...
    prepareData: function(data, config) {
      controlCount = Math.max.apply(null, data.map(function(item) {
      ...
      var columns = [
          ...
          },
          {
            ...
            onChange: function(rowIndex, newValue, api) {
              ...
              if (qcType) {
                ...
                selectedInstrument = Utils.array.findFirstOrNull(function(instrument) {
                  return instrument.instrumentModelId === qcType.instrumentModelId && instrument.name === selectedInstrumentName;
                }, config.instruments);
              }
              var instrumentRequired = qcType && qcType.instrumentModelId;
              api.updateField(rowIndex, 'instrumentId', {
                source: instrumentRequired ? config.instruments.filter(function(instrument) {
                  return instrument.instrumentModelId === qcType.instrumentModelId
                      && (!instrument.dateDecommissioned || (selectedInstrument && instrument.id === selectedInstrument.id));
                }) : [],
            ...
            getData: function(qc) {
              ...
              return Utils.array.findUniqueOrThrow(Utils.array.idPredicate(qc.instrumentId), config.instruments).name;
            },
            ...
            getData: function(qc) {
              ...
              return Utils.array.findUniqueOrThrow(Utils.array.idPredicate(qc.kitDescriptorId), Constants.kitDescriptors).name;
            },
            source: [], // set in QC type onChange
            getItemLabel: Utils.array.getName,
            getItemValue: Utils.array.getId,
        ...
        (function() {
          ...
          columns.push({
            ...
            getData: function(qc) {
              ...
              var qcType = Utils.array.findUniqueOrThrow(Utils.array.idPredicate(qc.qcTypeId), Constants.qcTypes);
              var control = Utils.array.findUniqueOrThrow(Utils.array.idPredicate(qc.controls[index].controlId), qcType.controls);

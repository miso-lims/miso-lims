ListTarget.samplevalidrelationship = (function($) {
    ...
    createBulkActions: function(config, projectId) {
      return !config.isAdmin || config.showSide === 'child' ? [] : [{
        ...
        action: function(items) {
          Utils.showDialog("Un/Archive Relationships", "OK", [{
            label: 'Archived',
            property: 'archived',
            type: 'checkbox'
          }], function(results) {
    ...
    createStaticActions: function(config, projectId) {
      return !config.isAdmin || config.showSide === 'child' ? [] : [{
        ...
        handler: function() {
          ...
          relationshipOptions = removeExistingRelationships(relationshipOptions, config.listId);
          if (!relationshipOptions.length) {
            Utils.showOkDialog('Error', ['No potential classes for new relationship']);
            return;
          }
          Utils.showWizardDialog('Add Parent Relationship', relationshipOptions.map(function(sampleClass) {
            return {
  ...
  function getSampleClassesForCategories(categories) {
    ...
    categories.forEach(function(category) {
      classes = classes.concat(Constants.sampleClasses.filter(function(sampleClass) {
  ...
  function removeExistingRelationships(relationshipOptions, listId) {
    var existingIds = $('#' + listId).dataTable().fnGetData().map(function(item) {

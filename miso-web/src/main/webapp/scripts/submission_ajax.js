...
var Submission = Submission || {
  saveSubmission: function(submissionId, form) {
    ...
    Fluxion.doAjax('submissionControllerHelperService', 'saveSubmission', {
  ...
  validateSubmissionMetadata: function(submissionId) {
    Fluxion.doAjax('submissionControllerHelperService', 'validateSubmissionMetadata', {
      'submissionId': submissionId,
      'url': ajaxurl
    }, {
      'doOnSuccess': function(json) {
  ...
  submitSubmissionMetadata: function(submissionId) {
    Fluxion.doAjax('submissionControllerHelperService', 'submitSubmissionMetadata', {
      'submissionId': submissionId,
      'url': ajaxurl
    }, {
      'doOnSuccess': function(json) {
  ...
  submitSequenceData: function(submissionId) {
    ...
    Fluxion.doAjax('submissionControllerHelperService', 'submitSequenceData', {
      'submissionId': submissionId,
      'url': ajaxurl
    }, {
      'doOnSuccess': function(json) {
...
Submission.ui = {
  openSubmissionProjectNodes: function(submissionId) {
    var self = this;
    Fluxion.doAjax('submissionControllerHelperService', 'openSubmissionProjectNodes', {
      'submissionId': submissionId,
      'url': ajaxurl
    }, {
      'doOnSuccess': function(json) {
  ...
  populateSubmissionProject: function(projectId, submissionId) {
    if (jQuery('#project' + projectId).hasClass("jstree-closed")) {
      Fluxion.doAjax('submissionControllerHelperService', 'populateSubmissionProject', {
        'projectId': projectId,
        'submissionId': submissionId,
        'url': ajaxurl
      }, {
  ...
  previewSubmissionMetadata: function(submissionId) {
    Fluxion.doAjax('submissionControllerHelperService', 'previewSubmissionMetadata', {
      'submissionId': submissionId,
      'url': ajaxurl
    }, {
      'doOnSuccess': function(json) {
  ...
  downloadSubmissionMetadata: function(submissionId) {
    Fluxion.doAjax('submissionControllerHelperService', 'downloadSubmissionMetadata', {
      'submissionId': submissionId,
      'url': ajaxurl
    }, {
      'doOnSuccess': function(json) {
  ...
  displayUploadProgress: function(submissionId) {
    Fluxion.doAjax('submissionControllerHelperService', 'checkUploadProgress', {
      'submissionId': submissionId,
      'url': ajaxurl
    }, {
      'doOnSuccess': function(json) {
        ...
        for (var i = 0; i < json.uploadJobs.length; i++) {
          jQuery('#submissionreport').append(
              "<tr><td style='width: 200px;'>" + json.uploadJobs[i].filename + ":</td>" + "<td style='width: 200px;'><div id='progressbar"
                  + i + "'></div></td>" + "<td>" + json.uploadJobs[i].percent + "%</td></tr>");
        }
        jQuery('#submissionreport').append("</table>");
        for (var j = 0; j < json.uploadJobs.length; j++) {
          jQuery("#progressbar" + j).progressbar({
            value: json.uploadJobs[j].percent
          });
        }
      }
    });
  ...
  actuallyUpdateProgress: function(json) {
    for (var i = 0; i < json.uploadJobs.length; i++) {
      jQuery("#progressbar" + i).progressbar({
        value: json.uploadJobs[i].percent
      });
    }
  },

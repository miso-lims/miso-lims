      ...
      return [BulkUtils.columns.simpleAlias(255), {
        ...
        onChange: function(rowIndex, newValue, api) {
          ...
          if (design) {
            ...
            code = Utils.array.findUniqueOrThrow(Utils.array.idPredicate(design.designCodeId),
        ...
        onChange: function(rowIndex, newValue, api) {
          ...
          api.updateField(rowIndex, 'libraryTypeId', {
            source: Constants.libraryTypes.filter(function(x) {
              return x.platform === platformType.name
          ...
          api.updateField(rowIndex, 'kitDescriptorId', {
            source: Constants.kitDescriptors.filter(function(x) {
              return x.platformType === platformType.key && x.kitType === 'Library'
          ...
          api.updateField(rowIndex, 'indexFamilyId', {
            source: Constants.indexFamilies.filter(function(x) {
              return x.platformType === platformType.name
        ...
        getData: function(item, api) {
          if (item.libraryTypeId) {
            var libraryType = Utils.array.findUniqueOrThrow(Utils.array.idPredicate(item.libraryTypeId),
        ...
        getData: function(item, api) {
          if (item.selectionId) {
            var selection = Utils.array.findUniqueOrThrow(Utils.array.idPredicate(item.selectionId),
        ...
        getData: function(item, api) {
          if (item.strategyId) {
            var strategy = Utils.array.findUniqueOrThrow(Utils.array.idPredicate(item.strategyId),
        ...
        getData: function(item, api) {
          if (item.kitDescriptorId) {
            var kit = Utils.array.findUniqueOrThrow(Utils.array.idPredicate(item.kitDescriptorId),
        ...
        getData: function(item, api) {
          if (item.indexFamilyId) {
            var family = Utils.array.findUniqueOrThrow(Utils.array.idPredicate(item.indexFamilyId),

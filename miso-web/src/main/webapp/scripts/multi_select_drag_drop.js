        ...
        //OK so they are selectable. now I need to make them draggable, in such a way that they pick up their friends when dragged. hmmm how do I do that?
          .draggable({
            helper: function () {
              ...
              var jQueryhelper = jQuery('<div dds_list="' + list + '"><div style="margin-top:-' + jQuery.fn.drag_drop_selectable.getMarginForDragging(jQueryclicked) + 'px;" /></div>').append(jQuery.fn.drag_drop_selectable.getSelectedForDragging(jQueryclicked.attr('dds')));
      ...
      jQuerylist.droppable({
        drop: function (e, ui) {
          ...
          if (jQuery.fn.drag_drop_selectable.settings[oldlist] && typeof(jQuery.fn.drag_drop_selectable.settings[oldlist].onListChange) == 'function') {
            setTimeout(function () {
              jQuery.fn.drag_drop_selectable.settings[oldlist].onListChange(jQuery('ul[dds=' + oldlist + ']'));
            }, 50);
          }
          if (jQuery.fn.drag_drop_selectable.settings[list_id] && typeof(jQuery.fn.drag_drop_selectable.settings[list_id].onListChange) == 'function') {
            setTimeout(function () {
              jQuery.fn.drag_drop_selectable.settings[list_id].onListChange(jQuery('ul[dds=' + list_id + ']'));
            }, 50);
          }
        },
        accept: function (d) {

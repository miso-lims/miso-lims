...
FormTarget.poolorder = (function($) {
    ...
    getSections: function(config, object) {
      ...
      return [{
        ...
        fields: [{
          ...
          include: !platform,
          onChange: function(newValue, form) {
            var platform = !newValue ? null : Utils.array.findUniqueOrThrow(Utils.array.namePredicate(newValue), Constants.platformTypes);
  ...
  function getPlatform(order) {
    ...
    return platformName == null ? null : Utils.array.findUniqueOrThrow(Utils.array.namePredicate(platformName), Constants.platformTypes);
  }
  ...
  function makeButton(text, onclick, form) {
    return $('<button>').addClass('ui-state-default').attr('type', 'button').text(text).click(ifSaved(form, onclick)).after(' ');
  }
  ...
  function createPool(form) {
    ...
    Utils.showConfirmDialog('Create Pool', 'Yes', ['Pool will contain the following aliquots and proportions.'].concat(
        orderAliquots.map(function(orderAliquot) {
          return '*' + orderAliquot.aliquot.alias + ' (' + orderAliquot.proportion + ')';
        })).concat(['Is this correct?']), function() {
  ...
  function linkPool(form) {
    Utils.showDialog('Fulfil Pool Order', 'Search', [{
      ...
      Utils.ajaxWithDialog('Searching...', 'GET', Urls.rest.pools.search + '?' + jQuery.param({
        ...
        if (!data || !data.length) {
          ...
          Utils.showWizardDialog('Choose Pool', data.map(function(pool) {
            return {
  ...
  function setPool(form, pool) {
    ...
    PoolOrder.getAliquots().forEach(
        function(orderAli) {
          var poolAli = Utils.array.findFirstOrNull(function(pooledElement) {
            return pooledElement.id === orderAli.aliquot.id
                || (pooledElement.parentAliquotIds && pooledElement.parentAliquotIds.indexOf(orderAli.aliquot.id) !== -1);
          }, pool.pooledElements);
          if (!poolAli) {
    ...
    if (notFound.length) {
      Utils.showOkDialog('Error', ['Selected pool does not contain the following library aliquots'].concat(notFound.map(function(
          notFoundAli) {
        return '* ' + notFoundAli.aliquot.name + ' (' + notFoundAli.aliquot.alias + ')';
      })));
    } else if (wrongProportions.length) {
      Utils.showConfirmDialog('Confirm Proportions', 'Confirm', ['Selected pool contains all of the required library aliquots, '
          + 'but the following are at different proportions'.concat(wrongProportions.map(function(wrongAli) {
            return '* ' + wrongAli.name + ' (' + wrongAli.alias + '): ' + wrongAli.proportion;
          })).concat(['Are you sure you wish to use this pool?'])], doSetPool(form, pool));
    } else {
      doSetPool(form, pool);
    }
  }
  ...
  function unlinkPool(form) {
    Utils.showConfirmDialog('Unlink Pool', 'Yes',
        ['Are you sure you wish to unlink the pool from this order? The order will be unfulfilled.'], function() {
          doSetPool(form, null)
        });
  }
  ...
  function linkSequencingOrder(form) {
    Utils.ajaxWithDialog("Finding Sequencing Orders", "GET", Urls.rest.sequencingOrders.search + '?' + jQuery.param({
      ...
      case 0:
        ...
        break;
      case 1:
        Utils.showConfirmDialog('Link Sequencing Order', 'Link', ['1 matching sequencing order found. Link to pool order?'], function() {
          setSequencingOrder(form, sequencingOrders[0]);
        });
        break;
      default:
        Utils.showWizardDialog('Link Sequencing Order', sequencingOrders.map(function(sequencingOrder) {
          return {
            name: 'Sequencing order ' + sequencingOrder.id + (sequencingOrder.description ? ' - ' + sequencingOrder.description : ''),
            handler: function() {
  ...
  function createSequencingOrder(form) {
    ...
    FormUtils.createFormDialog('Create Sequencing Order', sequencingOrder, 'sequencingorder', {}, function(result) {
      setSequencingOrder(form, result);
    });
  }

  function unlinkSequencingOrder(form) {
    Utils.showConfirmDialog('Unlink Sequencing Order', 'Yes',
        ['Are you sure you wish to unlink the sequencing order from this pool order? The order will be unfulfilled.'], function() {

ListTarget.metricsubcategory = (function() {
  ...
  return {
    ...
    createBulkActions: function(config) {
      ...
      if (config.isAdmin) {
        actions.push(ListUtils.createBulkDeleteAction(TYPE_LABEL, 'metricsubcategories', makeLabel));
      }
      return actions;
    },
    createStaticActions: function(config) {
      return config.isAdmin ? [ListUtils.createStaticAddAction(TYPE_LABEL, Urls.ui.metricSubcategories.bulkCreate, true)] : [];
    },
  ...
  function makeLabel(subcategory) {
    var category = Utils.array.findUniqueOrThrow(Utils.array.get('value'), Constants.metricCategories);

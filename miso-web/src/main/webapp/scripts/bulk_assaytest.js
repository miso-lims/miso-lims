      ...
      return [BulkUtils.columns.simpleAlias(50), {
        ...
        source: function(data, api) {
          return Constants.sampleClasses.filter(function(x) {
            return x.sampleCategory === 'Stock' && (!x.archived || x.id === data.extractionClassId);
          })
        },
        ...
        description: 'Describes how libraries are qualified before full depth sequencing - either via library aliquot '
        ...
        var col = BulkUtils.columns.makeBoolean('Repeat Per Timepoint', 'repeatPerTimepoint', true);
        col.include = Constants.isDetailedSample;
        col.description = 'If true, the test must be repeated for each timepoint received; if false, the test must '

      ...
      return [
        {
          ...
          onChange: function(rowIndex, newValue, api) {
            var model = Utils.array.findFirstOrNull(Utils.array.aliasPredicate(newValue), Constants.instrumentModels);

            var chemistryOptions = {
              disabled: false
            };
            if (!model || model.platformType !== 'ILLUMINA') {
              chemistryOptions.disabled = true,
              chemistryOptions.value = 'UNKNOWN'
            }
          ...
          title: 'Read 1 Length',
          description: 'For Illumina instruments, read 1 length should be greater than zero. For other platforms, read'
          ...
          title: 'Read 2 Length',
          description: 'For Illumina instruments, read 2 should be set to zero for single end, or greater than zero for'

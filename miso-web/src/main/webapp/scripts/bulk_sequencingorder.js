      ...
      return [{
        ...
        source: function(data, api) {
          return Constants.instrumentModels.filter(function(instrumentModel) {
            return instrumentModel.instrumentType === 'SEQUENCER'
        ...
        onChange: function(rowIndex, newValue, api) {
          if (newValue) {
            ...
            api.updateField(rowIndex, 'containerModelId', {
              source: !instrumentModel ? [] : instrumentModel.containerModels.filter(function(x) {
                return !x.archived;
              }),
              value: instrumentModel ? undefined : null
            });
            api.updateField(rowIndex, 'parameters.id', {
              source: !instrumentModel ? [] : Constants.sequencingParameters.filter(function(x) {
        ...
        source: function(data, api) {
          if (data.containerModelId) {
            ...
            return Constants.containerModels.filter(Utils.array.idPredicate(data.containerModelId))
          } else {
        ...
        source: function(data, api) {
          if (data.parameters && data.parameters.id) {
            // for display after save only - no bulk editing
            return Constants.sequencingParameters.filter(Utils.array.idPredicate(data.parameters.id))
          } else {
            return []; // set by container model inchange
          }
        },

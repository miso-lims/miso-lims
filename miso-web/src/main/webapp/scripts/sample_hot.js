...
Sample.hot = {
  ...
  makeHOT: function (startingValues) {
    ...
    var startRowsNumber = parseInt(window.prompt("How many samples would you like to create?"));
 ...
 addEmptyRow: function (numberToAdd) {
  ...
  getSampleGroups: function () {
    return this.sortByProperty(this.sampleOptions['sampleGroupsDtos'], 'id').map(this.getAlias);
  },
  ...
  buildSampleDtosFromData: function (obj) {
    ...
    // this hack is in place until Hibernatized sample is merged. TODO: get rid of it once hibernatized_sample is merged
    sample.name = 'SAM' + window.prompt('number for sample name?', '');
    ...
    // TODO: if (obj.kitDescriptor && obj.kitDescriptor.length) sample.sampleAdditionalInfo.prepKitId = Sample.hot.getIdFromAlias(obj.prepKitId, 'PREPKITTHING');
    ...
    if (Sample.hot.getSelectedCategory() == 'Analyte') {
      ...
      //if (obj.sampleGroupId) {
      //  sample.sampleAnalyte.sampleGroupId = this.getIdFromTHINGLIKELAB(obj.sampleGroupId, this.sampleOptions.sampleGroupsDtos);
      //}
  ...
  saveDetailedData: function () {
    ...
    if (document.getElementById('projectSelect').value === '' || document.getElementById('classDropdown').value === '') {
      alert('Make sure both Project and Sample Class are selected before saving.');
      return false;
    }
  ...
  savePlainData: function () {  
    ...
    if (document.getElementById('projectSelect').value === '') {
      alert('Make sure that a Project is selected before saving.');
      return false;
    }
    ...
    this.hotTable.validateCells(function (valid) {
      if (valid) {
        ...
        var hot1DataObject = JSON.parse(JSON.stringify({hotData: Sample.hot.hotTable.getData()}));
        var sampleData = Sample.hot.buildArrayFromObjectValue(hot1DataObject.hotData);

(function(IndexSearch, $, undefined) { // NOSONAR (paranoid assurance that undefined is undefined)
    ...
    if (!searchData.position1Indices.length && !searchData.position2Indices.length) {
      ...
      Utils.ajaxWithDialog('Index Search', 'POST', '/miso/rest/indices/search', searchData, function(data) {
        ...
        if (!data || !data.length) {
          ...
          var sequenceCount = searchData.position1Indices.length + searchData.position2Indices.length;
          data.sort(function(a, b) {
            var aMatches = a.position1Matches + a.position2Matches;
            var bMatches = b.position1Matches + b.position2Matches;
            return bMatches - aMatches;
          }).forEach(function(result) {
            results += result.indexFamily + ': ' + (result.position1Matches + result.position2Matches) + '/' + sequenceCount + '\n';
          });
        }

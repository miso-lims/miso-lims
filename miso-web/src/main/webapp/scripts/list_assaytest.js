ListTarget.assaytest = (function() {
  ...
  return {
    ...
    createBulkActions: function(config, projectId) {
      ...
      if (config.isAdmin) {
        if (!config.assayId) {
          // for list page
          actions.push(ListUtils.createBulkDeleteAction('Assay Tests', 'assaytests', Utils.array.getAlias));
    ...
    createStaticActions: function(config, projectId) {
      ...
      if (config.isAdmin) {
        if (!config.assayId) {
          // for list page
          actions.push(ListUtils.createStaticAddAction('Assay Tests', Urls.ui.assayTests.bulkCreate, true));
        } else if (config.pageMode === 'edit') {
          // for Edit Assay page
          actions.push({
            name: 'Add',
    ...
    createColumns: function(config, projectId) {
      return [{
        ...
        mRender: function(data, type, full) {
          var tissueType = Utils.array.findUniqueOrThrow(Utils.array.idPredicate(data), Constants.tissueTypes);
        ...
        mRender: function(data, type, full) {
          var code = Utils.array.findUniqueOrThrow(Utils.array.idPredicate(data), Constants.libraryDesignCodes);
        ...
        mRender: function(data, type, full) {
          ...
          if (data === 'ALIQUOT') {
            var code = Utils.array.findUniqueOrThrow(Utils.array.idPredicate(full.libraryQualificationDesignCodeId),
  ...
  function showAddTestDialog() {
    Utils.showWizardDialog('Add Assay Test', Constants.assayTests.map(function(test) {
      return {
        name: test.alias,
        handler: function() {
          Assay.addTest(test);

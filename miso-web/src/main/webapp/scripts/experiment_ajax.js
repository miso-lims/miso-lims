...
Experiment.ui = {
  wizardAddExperiment: function(form) {
    Fluxion.doAjax('experimentWizardControllerHelperService', 'addStudyExperiment', {
  ...
  addExperimentForm: function(id) {
    Fluxion.doAjax('experimentWizardControllerHelperService', 'addExperimentForm', {
      'newid': id,
      'url': ajaxurl
    }, {
      'doOnSuccess': function(json) {
  ...
  createListingExperimentsTable: function() {
    ...
    Fluxion.doAjax('experimentControllerHelperService', 'listExperimentsDataTable', {
      'url': ajaxurl
    }, {
      'doOnSuccess': function(json) {
...
Experiment.pool = {
  experimentSelectPool: function(p, newid) {
    ...
    if (poolSel.is(':empty')) {
      ...
      newpool
          .append("<input type='hidden' id='pool" + pool.attr("bind") + "' name='pool" + newid + "' value='" + pool.attr("bind") + "'/>");
      newpool.remove('.pType');
      newpool
          .append("<span style='position: absolute; top: 0; right: 0;' onclick='Experiment.pool.confirmPoolRemove(this);' class='float-right ui-icon ui-icon-circle-close'></span>");
  ...
  loadPoolsByPlatform: function(select, id) {
    ...
    Fluxion.doAjax('experimentWizardControllerHelperService', 'loadPoolsByPlatform', {
      'platformId': jQuery(select).val(),
      'newid': id,
      'url': ajaxurl
    }, {
      'doOnSuccess': function(json) {
...
Experiment.kit = {
  lookupKitByIdentificationBarcode: function(barcode) {
    Fluxion.doAjax('experimentControllerHelperService', 'lookupKitByIdentificationBarcode', {
      'barcode': barcode,
      'url': ajaxurl
    }, {
      'doOnSuccess': fillKitSelector
    } // ?? fillKitSelector is not defined anywhere in the project.
    );
  },

  lookupKitByLotNumber: function(lotNumber) {
    Fluxion.doAjax('experimentControllerHelperService', 'lookupKitByLotNumber', {
      'lotNumber': lotNumber,
      'url': ajaxurl
    }, {
      'doOnSuccess': fillKitSelector
  ...
  lookupKitDescriptorByPartNumber: function(t) {
    var func = function() {
      Fluxion.doAjax('experimentControllerHelperService', 'lookupKitDescriptorByPartNumber', {
        'partNumber': t.value,
        'url': ajaxurl
      }, {
        'doOnSuccess': function(json) {
  ...
  showLibraryKitDialog: function(experimentId, multiplexed) {
    var self = this;
    Fluxion.doAjax('experimentControllerHelperService', 'getLibraryKitDescriptors', {
  ...
  processLibraryKitList: function(json) {
    ...
    for (var k = 0; k < libraryKits.length; k++) {
      dialogText += "<option partNumber='" + libraryKits[k].partNumber + "' value='" + libraryKits[k].id + "'>" + libraryKits[k].name
          + "</option>";
    }
    ...
    if (multiplexed == "true") {
      ...
      for (var m = 0; m < multiplexKits.length; m++) {
        dialogText += "<option partNumber='" + multiplexKits[m].partNumber + "' value='" + multiplexKits[m].id + "'>"
            + multiplexKits[m].name + "</option>";
      }
      dialogText += "</select><br/>"
    ...
    dialogText += "<br/>" + "<label for='lotNumber'>Lot Number / Barcode</label>"
          ...
          buttons: {
            "Add Library Kit": function() {
              ...
              if (jQuery("#addKitForm").validate().form()) {
                if (multiplexed == "true") {
                  self.addLibraryKit(experimentId, jQuery('#kitDescriptor').val(), jQuery('#partNumber').val(), jQuery(
                      '#multiplexingKitType').val(), jQuery('#multiplexingKitBarcode').val(), jQuery('#lotNumber').val());
                } else {
                  self.addLibraryKit(experimentId, jQuery('#kitDescriptor').val(), jQuery('#partNumber').val(), undefined, undefined,
                      jQuery('#lotNumber').val());
                }
  ...
  addLibraryKit: function(experimentId, kitDescriptorId, partNumber, mplexKitType, mplexKitBarcode, lotNumber) {
    if (mplexKitType === undefined || mplexKitBarcode === undefined) {
      Fluxion.doAjax('experimentControllerHelperService', 'addLibraryKit', {
        'experimentId': experimentId,
        'kitDescriptor': kitDescriptorId,
  ...
  showEmPcrKitDialog: function(experimentId) {
    var self = this;
    Fluxion.doAjax('experimentControllerHelperService', 'getEmPcrKitDescriptors', {
      'experimentId': experimentId,
      'url': ajaxurl
    }, {
  ...
  addEmPcrKit: function(experimentId, kitDescriptorId, partNumber, lotNumber) {
    Fluxion.doAjax('experimentControllerHelperService', 'addEmPcrKit', {
      'experimentId': experimentId,
      'kitDescriptor': kitDescriptorId,
      'partNumber': partNumber,
      'lotNumber': lotNumber,
  ...
  showClusteringKitDialog: function(experimentId) {
    var self = this;
    Fluxion.doAjax('experimentControllerHelperService', 'getClusteringKitDescriptors', {
      'experimentId': experimentId,
      'url': ajaxurl
    }, {
  ...
  addClusteringKit: function(experimentId, kitDescriptorId, partNumber, lotNumber) {
    Fluxion.doAjax('experimentControllerHelperService', 'addClusteringKit', {
      'experimentId': experimentId,
      'kitDescriptor': kitDescriptorId,
      'partNumber': partNumber,
      'lotNumber': lotNumber,
  ...
  showSequencingKitDialog: function(experimentId) {
    var self = this;
    Fluxion.doAjax('experimentControllerHelperService', 'getSequencingKitDescriptors', {
      'experimentId': experimentId,
      'url': ajaxurl
    }, {
  ...
  addSequencingKit: function(experimentId, kitDescriptorId, partNumber, lotNumber) {
    Fluxion.doAjax('experimentControllerHelperService', 'addSequencingKit', {
      'experimentId': experimentId,
      'kitDescriptor': kitDescriptorId,
      'partNumber': partNumber,
      'lotNumber': lotNumber,

...
<h1>
  <c:choose><c:when test="${pageMode eq 'create'}">Create</c:when><c:otherwise>Edit</c:otherwise></c:choose> Requisition
        ...
        <ul class="sddm">
          <li>
            ...
            <div id="notesmenu"
              ...
              <a onclick="Utils.notes.showNoteDialog('requisition', ${requisition.id});" href="javascript:void(0);" class="add">Add Note</a>
            </div>
          </li>
        </ul>
                ...
                <span class="float-right" style="font-weight:bold; color:#C0C0C0;">${note.owner.loginName}
                  <c:if test="${miso:isCurrentUser(note.owner.loginName) or miso:isAdmin()}">
                    <span style="color:#000000">
                      <a href='#' onclick="Utils.notes.deleteNote('requisition', '${requisition.id}', '${note.id}'); return false;">

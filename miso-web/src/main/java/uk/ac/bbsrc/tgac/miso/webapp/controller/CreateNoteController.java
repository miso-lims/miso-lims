...
public class CreateNoteController {
   ...
   * Request request = requestManager.getRequestById(requestId); if (!request.userCanWrite(user)) { throw new
   * SecurityException("Permission denied."); } Note note = request.createNote(user); note.setText(text);
   * note.setInternalOnly(internalOnly); requestManager.saveRequest(request); session.setComplete(); model.clear(); return
   * "redirect:/miso/request/view/" + requestId; } catch (IOException ex) { if (log.isDebugEnabled()) { log.debug("Failed to create note",
   * ex); } throw ex; } }
   */
}

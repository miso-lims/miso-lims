 ...
 * @since 0.1.2
 */
public class PrimaryAnalysisCompletedResponderService extends AbstractResponderService<ProjectOverviewEvent> {
  protected static final Logger log = LoggerFactory.getLogger(PrimaryAnalysisCompletedResponderService.class);

  private Set<AlerterService> alerterServices = new HashSet<AlerterService>();

  public PrimaryAnalysisCompletedResponderService() {
  }
  ...
  public boolean respondsTo(ProjectOverviewEvent event) {
    ...
    if (event.getEventType().equals(MisoEventType.PRIMARY_ANALYSIS_COMPLETED) && po.getPrimaryAnalysisCompleted()) {
  ...
  public void generateResponse(ProjectOverviewEvent event) {
    ...
    for (User user : po.getWatchers()) {
      ...
      at.append("The primary analysis related to this Project has completed: " + po.getProject().getAlias() + " (" + event.getEventMessage()

...
CONFIG CHANGE: External service links can be added to Edit Project pages. This can be configured in 
`miso.properties` file (usually located at `<tomcat-home-directory>/conf/Catalina/localhost/miso.properties`) 
by adding the property `miso.project.report.links`, like so:
```
...
CONFIG CHANGE: External service links can be added to Edit Run pages. This can be configured in the
`miso.properties` file (usually located at `<tomcat-home-directory>/conf/Catalina/localhost/miso.properties`)

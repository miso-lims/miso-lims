 ...
 * --------------------------------
 *
 * {@link org.springframework.integration.core.MessageSource} that creates messages from a file system directory. To
 ...
 * is ready. The default {@link org.springframework.integration.file.filters.AcceptOnceFileListFilter}
 ...
 * a known suffix), composed with the default {@link org.springframework.integration.file.filters.AcceptOnceFileListFilter}
 * would allow for this.
 * <p/>
 * A {@link java.util.Comparator} can be used to ensure internal ordering of the Files in
 * a {@link java.util.concurrent.PriorityBlockingQueue}. This does not provide the same guarantees as
 * a {@link org.springframework.integration.aggregator.ResequencingMessageGroupProcessor}, but in cases where writing files
...
public class MultiFileQueueMessageSource extends IntegrationObjectSupport implements MessageSource<Set<File>> {
   ...
   * with no bounds is used. In most cases a customized {@link org.springframework.integration.file.filters.FileListFilter} will
  ...
  /**
   * Optional. Sets a {@link org.springframework.integration.file.FileLocker} to be used to guard files against
   ...
   * refreshed with the latest content of the input directories on each poll.
   * <p/>
   ...
   *
   * @param scanEachPoll whether or not the component should re-scan (as opposed to not
  ...
  protected void onInit() {
    ...
    for (File directory : directories) {
      ...
      Assert.isTrue(directory.isDirectory(),
                    "Source path [" + directory + "] does not point to a directory.");
      Assert.isTrue(directory.canRead(),
                    "Source directory [" + directory + "] is not readable.");
    }
  }
   ...
   *
   * @param failedMessage the {@link org.springframework.integration.Message} that failed
   */

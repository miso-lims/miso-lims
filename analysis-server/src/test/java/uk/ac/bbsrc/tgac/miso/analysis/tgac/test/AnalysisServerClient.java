...
public class AnalysisServerClient {
  ...
  public static void main(String[] args) {
    try {
      ...
      q2.put("query", "getPipelines");
      String q2s = q2.toString();
      log.info("QUERY: " + q2s);
      String q2rs = IntegrationUtils.sendMessage(IntegrationUtils.prepareSocket("norwich", 7898), q2s);
      log.info("RESULT: " + q2rs);
*/
      ...
      params.put("sample-sheet-string", "FCID,Lane,SampleID,SampleRef,Index,Description,Control,Recipe,Operator,Project\\n" +
      ...
      if (!"".equals(response)) {
        ...
        if (!r.isEmpty()) {
          if (r.size() == 1 && r.getJSONObject(0).has("error")) {
            ...
            log.error(error);
            throw new InterrogationException("Analysis query returned an error: " + error);
          }
